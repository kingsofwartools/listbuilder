function ViewOptions() {
  // view the options for the currently edited unit
  View("options");
  var o = $("#options");
  ...
  if (u.options.length > 0) {
    // show the on/off buttons for each option
    var ol = E("ul").appendTo(
      E("div")
...

  {
    ...
    _sections.forEach((section, sectionIndex) => {
      section.units.forEach(unit => {
        ...
        if (unit[0] && unit[0].item && unit[0].item !== _unit.item)
          artefactsUsedElsewhere.push(parseInt(unit[0].item));
      });
    });
    var parsedArtefacts = _artefacts.map((artefact, index) => {
      const parsedArtefact = { ...artefact };
      parsedArtefact.index = index;
      if (typeof artefact.cost === "object")
        parsedArtefact.cost = artefact.cost[u.unitType[_unit.ut]];
      return parsedArtefact;
    });
    const displayArtefactDropdown =
      !(["W", "M", "Ti"].includes(u.unitType[_unit.ut]) || u.limit) &&
      (!_sections[0] || _unit.fi === _sections[0].fi);
    if (displayArtefactDropdown) {
      var d = E("div")
        .attr("class", "dropdown theme-dropdown clearfix")
        .appendTo(tf);
      E("button")
        .attr("id", "magicitems")
        .attr("type", "button")
        ...
        .text("Item:")
        .appendTo(d);
      E("span", { id: "unitArtefact" })
        .text(
          " " +
            parsedArtefacts[_unit.item].name +
            " (+" +
            parsedArtefacts[_unit.item].cost +
            " pts)"
        )
        .appendTo(d);
      var ul = E("ul")
    ...
    const parsedFilteredArtefacts = parsedArtefacts
      .filter(artefact => {
    ...
    if (_cur.suii != -1) {
      ...
      buttons.append(
        E("div", { class: "btn-group", role: "group" }).append(
          E("button", { class: "btn btn-warning" })
            .click(DeleteUnitClick)
            .text("Delete")
        )
      );
    }
...
function DeleteUnitClick() {
  // prompt for deletion of a unit
  ...
  p.append(
    E("div", { id: "deleteName" }).text(u.name + " (" + u.models[nfo.ut] + ")")
  );
  if (_cur.sui == 0)
    p.append(
      "Deleting a core Regiment or Horde will also delete any attached Troop, Monster, War Engine, or Hero."
    );
...
function DeleteUnit() {
  ...
  if (_cur.sui == 0) _sections.splice(_cur.si, 1);
  ViewMain();
}

function TogglePrompt() {
  // close/open the prompt
  toggle("prompt");
  toggle("options");
}
...
function toggle(view) {
  $("#view_" + view).css(
    "display",
    $("#view_" + view).css("display") == "block" ? "none" : "block"
  );
}
function Cancel() {
  if (_cur.suii == -1) ViewUnits(_unit.fi, _uTypes);
  else ViewMain();
}
...
function SetOption() {
  ...
  if (
    ($.inArray(oi, _unit.options) == -1 && on == 0) ||
...
function Save() {
  // save the final options, apply to army list, go to list view
  var mc = $.inArray(
    _catalog[_unit.fi].units[_unit.ui].unitType[_unit.ut],
    _mCountsIndex
  );
  ...
  if (unitObj.irregular == 1) sui = 1; // if irregular, it's always a troop selection
  if (_cur.fi == -1) _cur.fi = _unit.fi;

  var unit = CopyUnit(_unit);
  if (_cur.suii == -1)
    // new unit
    _section.units[sui].push(unit);
  else _section.units[sui][_cur.suii] = unit;
  ...
  } else _sections[_cur.si] = section;
